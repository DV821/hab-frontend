...
import { Loader2, ArrowLeft, MapPin, AlertTriangle, CheckCircle, Crown } from "lucide-react"
...
export default function PredictionPage({
  ...
  const [lat, setLat] = useState(27.6133)
  const [lon, setLon] = useState(-82.7391)
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="font-semibold">
                    {tierConfig.model} • {tierConfig.processingTime} • {tierConfig.predictionDays}-day predictions
                  </div>
          ...
          <Card className="backdrop-blur-sm bg-white/80 border-0">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="date">Start Date for {tierConfig.predictionDays}-day Window</Label>
                  <Input
                    id="date"
                    type="date"
                    value={selectedDate}
                  ...
                  <p className="text-sm text-gray-600">
                    The model will predict HAB status for {tierConfig.predictionDays} days after this date.
                  </p>
                </div>
          ...
          <Card className="backdrop-blur-sm bg-white/80 border-0 flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col min-h-0">
              <div className="flex-1 min-h-[280px]">
                <InteractiveMap lat={lat} lon={lon} onLocationSelect={handleLocationSelect} />
              </div>
            </CardContent>
          </Card>
        </div>
              ...
              <CardTitle className="text-teal-700 flex items-center gap-2">
                ...
                {prediction.location && (
                  <span className="text-sm font-normal text-gray-600 ml-2">
                    at ({prediction.location.latitude.toFixed(4)}, {prediction.location.longitude.toFixed(4)})
                  </span>
                )}
              </CardTitle>
            </CardHeader>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h3 className="font-bold text-xl">
                        {prediction.predicted_label === "toxic" ? "TOXIC" : "NON-TOXIC"}
                      </h3>
                    </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-gray-700 mb-3">Confidence Scores</h4>
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-sm font-medium">Non-Toxic</span>
                          <span className="text-sm font-mono">
                            {(Number.parseFloat(prediction.confidence_scores.non_toxic) * 100).toFixed(2)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className="bg-green-500 h-2 rounded-full transition-all duration-500"
                            style={{ width: `${Number.parseFloat(prediction.confidence_scores.non_toxic) * 100}%` }}
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-sm font-mono">
                            {(Number.parseFloat(prediction.confidence_scores.toxic) * 100).toFixed(2)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className="bg-red-500 h-2 rounded-full transition-all duration-500"
                            style={{ width: `${Number.parseFloat(prediction.confidence_scores.toxic) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">{prediction.prediction_for_date}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Location:</span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{prediction.processing_time || tierConfig.processingTime}</span>
                      </div>
                      <div className="flex justify-between">
                  ...
                  <details className="bg-gray-50 p-4 rounded-lg">
                    <summary className="cursor-pointer font-semibold text-gray-700">View Raw API Response</summary>
